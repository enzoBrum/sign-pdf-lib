...
function updateByteRange(incrementalPdf: Buffer, initialPdfLength: number): Buffer | undefined {
  ...
  return Buffer.concat([incrementalPdf.subarray(0, startOfByteRange), byteRangeBuffer, incrementalPdf.subarray(endOfByteRange)]);
}
...
export class PdfSigningDocument {
  ...
  async saveAsync(): Promise<Buffer> {
    ...
    return Buffer.concat([this.#pdf, incrementalPdf]);
  }
  ...
  getSignatures(): PDFDict[] {
    ...
    return formFields
      ...
      .filter((ref) => {
        ...
        return (
          dict.lookupMaybe(PDFName.of("FT"), PDFName) == PDFName.of("Sig") &&
